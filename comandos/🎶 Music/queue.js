    ...
    run: async (client, message, args, prefix) => {
        ...
        for (let i = 0; i < limite; i++) {
            ...
            let embed = new Discord.EmbedBuilder()
                .setTitle(`🎶 ${queue.songs.length} ${queue.songs.length > 1 ? "Canciones en cola" : "Canción en cola"}`)
            ...
            if (queue.songs.length > 1) embed.addFields([{name: `💿 Reproduciendo`, value: `**[\`${queue.songs[0].name}\`](${queue.songs[0].url})**`}, ])
        ...
        async function paginacion() {
            ...
            collector.on("collect", async b => {
                ...
                if (b?.user.id !== message.author.id) return b?.reply({
                    embeds: [new Discord.EmbedBuilder()
                        .setTitle(`Solo la persona que ha escrito lo siguiente puede cambiar de páginas`)
                        
                        .setDescription(`\`${prefix}queue\``)
                        .setThumbnail('https://imgur.com/sjQTlh9.png')
                        .setColor("#E24C4B")
                        .setFooter({ text: 'Powered By Welben', iconURL: 'https://imgur.com/TsKFaHl.png' })
                    ], ephemeral: true
